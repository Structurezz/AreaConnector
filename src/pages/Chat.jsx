        ...
        <div className="px-4 py-2.5 rounded-2xl rounded-bl-sm flex items-center gap-1.5"
          ...
          {[0, 150, 300].map(d => (
            <span key={d} className="w-1.5 h-1.5 rounded-full animate-bounce"
              style={{ background: 'rgba(245,158,11,0.6)', animationDelay: `${d}ms` }} />
          ))}
        </div>
        <span className="text-xs pl-1 mt-0.5" style={{ color: '#D97706', opacity: 0.5 }}>typing...</span>
      </div>
    </div>
  );
}
    ...
    return (
      <div className="flex flex-col items-end mb-0.5">
        <div className="flex items-end gap-1.5 max-w-[72%]">
          ...
          <div className="bg-emerald-600 text-white font-medium px-3.5 py-2 rounded-2xl rounded-br-sm text-sm leading-relaxed">
            {msg.content}
          </div>
        </div>
      </div>
  ...
  return (
    <div className="flex items-end gap-2 mb-0.5">
      ...
      <div className="flex flex-col items-start max-w-[72%]">
        {showName && (
          <span className="text-xs mb-1 pl-1 font-medium"
            style={{ color: isNkechi ? '#D97706' : '#94A3B8' }}>
            {senderName}
            {isNkechi && <span className="ml-1.5 text-xs font-normal" style={{ color: 'rgba(217,119,6,0.6)' }}>· AI Moderator</span>}
          </span>
        )}
        <div className="flex items-end gap-1.5">
          <div className="px-3.5 py-2 rounded-2xl text-sm leading-relaxed rounded-tl-sm"
            style={isNkechi
              ? { background: '#FFFBEB', border: '1px solid rgba(245,158,11,0.2)', color: '#92400E' }
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4"
      ...
      <div className="glass-card w-full max-w-sm overflow-hidden" onClick={e => e.stopPropagation()}>
        <div className="flex items-center gap-3 px-4 py-3.5"
          ...
          <MessageSquare size={15} className="text-emerald-600 flex-shrink-0" />
          <span className="font-semibold text-sm flex-1" style={{ color: '#0F172A' }}>New Direct Message</span>
          <button onClick={onClose} className="transition-colors" style={{ color: '#94A3B8' }}><X size={16} /></button>
        </div>
        <div className="p-4">
          ...
          <div className="space-y-0.5 max-h-64 overflow-y-auto">
            {loading ? (
              ...
              <p className="text-center text-sm py-6" style={{ color: '#94A3B8' }}>
                {query ? 'No users found' : 'No residents in this estate'}
              </p>
            ) : (
              users.map(u => (
...
function Thread({ thread, currentUser, subscribe, emit, estateName }) {
  ...
  return (
    <>
      ...
      {/* Input */}
      <div className="flex-shrink-0 px-4 py-3" style={{ borderTop: '1px solid #E2E8F0', background: '#FFFFFF' }}>
...
export default function Chat() {
  ...
  return (
    <div className="flex h-[calc(100vh-5rem)] -m-4 md:-m-6 lg:-m-8 overflow-hidden">
      ...
      <div className={`flex flex-col w-full md:w-72 flex-shrink-0 ${
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-0.5">
          ...
          {conversations.length > 0 && (
            <div className="px-3 pt-3 pb-1.5">
              <p className="text-xs uppercase tracking-widest font-medium" style={{ color: '#CBD5E1' }}>Direct Messages</p>
            </div>
          )}

          {convoLoading && conversations.length === 0 ? (
            <div className="flex justify-center py-6">
              <RefreshCw size={14} className="animate-spin" style={{ color: '#CBD5E1' }} />
            </div>
          ) : (
            conversations.map(c => (
              <ConvoItem
          ...
          {!convoLoading && conversations.length === 0 && (
            <div className="text-center py-8 px-4">
              <MessageSquare size={22} className="mx-auto mb-2" style={{ color: '#E2E8F0' }} />
              <p className="text-xs" style={{ color: '#94A3B8' }}>No direct messages yet</p>
              <button onClick={() => setShowNewDM(true)}
                className="mt-3 text-xs transition-colors" style={{ color: '#10B981' }}>
      ...
      <div className={`flex flex-col flex-1 min-w-0 ${!mobileShowThread ? 'hidden md:flex' : 'flex'}`}>
        <button
          className="md:hidden flex items-center gap-2 px-4 py-2.5 text-sm transition-colors flex-shrink-0"
          style={{ borderBottom: '1px solid #E2E8F0', background: '#FFFFFF', color: '#94A3B8' }}
